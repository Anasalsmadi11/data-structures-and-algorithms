...
const validatePin = (pin) => {
  // Solution code here...
  const regex = /^\d{4}$/; //pattern for four numerical digits
...
const validateWord = (word) => {
  ...
  if (typeof word !== 'string' || word.length < 5 || word.length > 10 || /\d/.test(word)) {
    return false; // Return false if the input is not a string, has an invalid length, or contains a number
  }
...
const hasNumber = (string) => {
  // Solution code here...
  const regex = /[A-Za-z]+\d+/; // Regular expression pattern to match letters followed by digits
